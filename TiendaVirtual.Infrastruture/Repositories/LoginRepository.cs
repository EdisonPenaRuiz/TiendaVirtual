...
namespace TiendaVirtual.Infrastruture.Repositories
{
    public class LoginRepository:ILoginRepository
    {
       ...
       public async Task<RepuestasServidorGenericas<Core.Entities.Usuario>> LoginUsuario(string NombreUsuario, string Contrasena)
       {
            ...
            try
            {
                var usuarioLoguin = await _context.Usuarios.Where(x => x.NombreUsuario == NombreUsuario && x.Contrasena == Contrasena).Select(usuario => new Usuario {Nombre= usuario.Nombre,Apellido=usuario.Apellido,UsuarioId=usuario.UsuarioId}).ToListAsync();
